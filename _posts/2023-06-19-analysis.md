...
The chart above represents the percent of lifetime spent active for the various CKAN instances. The data is grouped by the year the instances were launched, with with the total number of instances per year listed atop each bar. For almost every year, we see that the average “lifespan” is above 80%.
